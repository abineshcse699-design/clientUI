...
export class Genie implements OnInit {
  ...
  ngOnInit() {
    // Set up search listener immediately with mock data
    this.setupSearchListener();

    // Optionally try to load data from API in the background (non-blocking)
    this.http.get<SpeciesDto[]>(`${this.API_BASE}/genie`).subscribe({
      next: data => {
      ...
      error: () => {
        // API not available - continue using mock data (already set up)
